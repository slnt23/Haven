...
_ENV_OVERRIDES: list[tuple[str, str]] = [
    ...
    ("MEMORY_TOKEN_BUDGET", "memory.memory_token_budget"),
    ...
    def load(*, overrides: dict[str, Any] | None = None) -> AppConfig:
        ...
        config = AppConfig(
            ...
            memory=MemoryConfig(
                ...
                memory_token_budget=int(app_raw.get("memory", {}).get("memory_token_budget", 800)),
                dedup_threshold=float(app_raw.get("memory", {}).get("dedup_threshold", 0.90)),
            ),
