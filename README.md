...
在项目根目录放置用户配置文件，只写想覆盖的字段即可：

| 文件 | 用途 |
|------|------|
| `.env` | API Key、邮箱密码等敏感信息 |
| `haven.yaml` | 框架参数（agent、RAG、email、memory 等） |
| `models.yaml` | 自定义/追加 LLM 模型 |
| `mcp.json` | MCP 服务器配置 |
...
| 目标 | 方式 | 代码量 |
|------|------|--------|
| 新增领域能力 / 人格 | `skills/` 下创建 `.md` 文件 | 零代码 |
...
```
├── .env
├── haven.yaml          # 可选：用户配置覆盖
├── models.yaml         # 可选：用户模型定义
├── mcp.json
├── skills/             # Skill 文件（.md，拖入即用）
├── src/forest/
│   ├── agents/         # Agent 实现
│   ├── cli/            # CLI 入口 & REPL
